...
public interface ProductoRepository extends JpaRepository<Producto, Integer> {
    ...
    @Query(value = "SELECT p.codigoGenerado as codigogenerado,s.nombre as nombre_sede,d.nombre,est.cantidad,i.precioMosqoy,est.fecha,est.estado\n" +
            " FROM producto p \n" +
            "            inner join denominacion d on p.idDenominacion = d.idDenominacion\n" +
            "            inner join inventario i on p.idProducto = i.idProducto\n" +
            "            inner join inventariosede invs on i.idInventario = invs.idInventario\n" +
    ...
    List<ProductosEstados> listaProductosEnviadosSede(int idSede);
